...
# ---------------------------------------------------------------------------
# workload-inspect -- CLI wiring only. inspect_workload() itself is tested
# against hand-built fake Kubernetes clients in test_kubernetes_inspect.py;
# here the point is option parsing, --json and exit codes, so both
# get_kubernetes_clients() and inspect_workload() are monkeypatched instead
# of touching a real cluster.
# ---------------------------------------------------------------------------


def _fake_workload_report(*, rollout_state="healthy"):
    return {
        "status": "ok",
        "deployment": {
            "name": "payments",
            "namespace": "payments",
            "desired_replicas": 2,
            "ready_replicas": 2 if rollout_state == "healthy" else None,
            "available_replicas": 2 if rollout_state == "healthy" else None,
            "updated_replicas": 2,
            "unavailable_replicas": None if rollout_state == "healthy" else 2,
            "conditions": [],
                ...
                "containers": [
                    {
                        "name": "payments",
                        "image": "busybox:1.36",
                        "ready": rollout_state == "healthy",
                        "restart_count": 0 if rollout_state == "healthy" else 4,
                        "state": "running" if rollout_state == "healthy" else "waiting",
                        "reason": None
                        if rollout_state == "healthy"
        ...
        if rollout_state == "healthy"
        else [
            {
                ...
                "involved_object": "payments-abc-1",
                "count": 6,
                "last_seen": "2026-08-05T19:22:18+00:00",
            }
        ],
    }


def test_workload_inspect_exits_0_for_a_healthy_rollout(monkeypatch):
    monkeypatch.setattr(
        cli_module, "get_kubernetes_clients", lambda **kwargs: (object(), object())
    )
    monkeypatch.setattr(
        cli_module, "inspect_workload", lambda *a, **k: _fake_workload_report()
    )

    result = runner.invoke(app, ["workload-inspect", "payments", "-n", "payments"])

    assert result.exit_code == 0
    assert "HEALTHY" in result.stdout


def test_workload_inspect_exits_1_for_an_unavailable_rollout(monkeypatch):
    monkeypatch.setattr(
        cli_module, "get_kubernetes_clients", lambda **kwargs: (object(), object())
    )
    monkeypatch.setattr(
        cli_module,
        "inspect_workload",
        lambda *a, **k: _fake_workload_report(rollout_state="unavailable"),
    )
...
def test_workload_inspect_json_prints_the_full_report(monkeypatch):
    monkeypatch.setattr(
        cli_module, "get_kubernetes_clients", lambda **kwargs: (object(), object())
    )
    monkeypatch.setattr(
    ...
    result = runner.invoke(
        app, ["workload-inspect", "payments", "-n", "payments", "--json"]
    )
    payload = json.loads(result.stdout)

    assert result.exit_code == 0
    assert payload["rollout_state"] == "healthy"
    assert payload["deployment"]["name"] == "payments"


def test_workload_inspect_exits_2_when_deployment_is_not_found(monkeypatch):
    monkeypatch.setattr(
        cli_module, "get_kubernetes_clients", lambda **kwargs: (object(), object())
    )
    monkeypatch.setattr(
        ...
        lambda *a, **k: {
            "status": "error",
            "error": "404",
            "message": "Deployment 'payments' not found in namespace 'payments'",
        },
    )
...
def test_workload_inspect_exits_2_when_kubeconfig_cannot_be_loaded(monkeypatch):
    def _raise_config_error(**kwargs):
        raise ConfigException("Invalid kube-config file. No configuration found.")

    monkeypatch.setattr(cli_module, "get_kubernetes_clients", _raise_config_error)

    result = runner.invoke(app, ["workload-inspect", "payments", "-n", "payments"])

    assert result.exit_code == 2
    assert "kube-config" in result.output


# ---------------------------------------------------------------------------
# restart-plan / restart-execute -- CLI wiring only. build_restart_plan()
# and execute_restart() are unit-tested against hand-built fake Kubernetes
# clients in test_kubernetes_restart.py; here the point is option parsing,
# --json and exit codes, so get_kubernetes_clients(), load_ops_config(),
# build_restart_plan() and execute_restart() are all monkeypatched instead
# of touching a real cluster.
# ---------------------------------------------------------------------------

_OPS_CONFIG = {
    "namespace_allowlist": ["checkout"],
    "cooldown_seconds": 120,
    "rollout_timeout_seconds": 60,
    "rollout_poll_interval_seconds": 2,
    "audit_log_path": "k8s-ops-audit.jsonl",
}


def _plannable_restart_plan():
    return RestartPlan(
        namespace="checkout",
        name="checkout-web",
        status="plannable",
        action=RestartAction(
            ...
            args={"name": "checkout-web", "namespace": "checkout", "body": {}},
        ),
        before_rollout_state="unavailable",
        dry_run_validated=True,
    )


def _not_allowed_restart_plan():
    return RestartPlan(
        namespace="billing",
        name="invoices",
        status="not_allowed",
...
def _mock_restart_plan_wiring(monkeypatch, *, plan):
    monkeypatch.setattr(
        cli_module, "get_kubernetes_clients", lambda **kwargs: (object(), object())
    )
    monkeypatch.setattr(cli_module, "load_ops_config", lambda path: dict(_OPS_CONFIG))
    monkeypatch.setattr(cli_module, "build_restart_plan", lambda *a, **k: plan)


def test_restart_plan_exits_0_for_a_plannable_restart(monkeypatch):
    _mock_restart_plan_wiring(monkeypatch, plan=_plannable_restart_plan())

    result = runner.invoke(app, ["restart-plan", "checkout-web", "-n", "checkout"])

    assert result.exit_code == 0
    assert "plannable" in result.stdout
    assert "dry_run=All, validated" in result.stdout


def test_restart_plan_exits_1_when_namespace_is_not_allowed(monkeypatch):
    _mock_restart_plan_wiring(monkeypatch, plan=_not_allowed_restart_plan())

    result = runner.invoke(app, ["restart-plan", "invoices", "-n", "billing"])

    assert result.exit_code == 1
    assert "not on the namespace allowlist" in result.stdout


def test_restart_plan_json_prints_the_full_plan(monkeypatch):
    _mock_restart_plan_wiring(monkeypatch, plan=_plannable_restart_plan())

    result = runner.invoke(
        app, ["restart-plan", "checkout-web", "-n", "checkout", "--json"]
    )
    payload = json.loads(result.stdout)

    assert result.exit_code == 0
    assert payload["plan"]["status"] == "plannable"
    assert payload["plan"]["dry_run_validated"] is True


def test_restart_plan_exits_2_when_kubeconfig_cannot_be_loaded(monkeypatch):
    def _raise_config_error(**kwargs):
        raise ConfigException("Invalid kube-config file. No configuration found.")
...
def _mock_restart_execute_wiring(monkeypatch, *, plan, execute_side_effect):
    monkeypatch.setattr(
        cli_module, "get_kubernetes_clients", lambda **kwargs: (object(), object())
    )
    monkeypatch.setattr(cli_module, "load_ops_config", lambda path: dict(_OPS_CONFIG))
    monkeypatch.setattr(cli_module, "build_restart_plan", lambda *a, **k: plan)
    monkeypatch.setattr(cli_module, "execute_restart", execute_side_effect)


def test_restart_execute_without_approve_refuses_and_shows_the_plan(monkeypatch):
    plan = _plannable_restart_plan()

    def _raise_not_approved(*a, **k):
        raise RestartNotApprovedError(plan)

    _mock_restart_execute_wiring(
        monkeypatch, plan=plan, execute_side_effect=_raise_not_approved
    )

    result = runner.invoke(app, ["restart-execute", "checkout-web", "-n", "checkout"])
...
def test_restart_execute_approve_exits_0_when_rollout_completes(monkeypatch):
    plan = _plannable_restart_plan()
    rolled_out_result = RestartResult(
        namespace="checkout",
        name="checkout-web",
        status="restarted",
        rollout_outcome="rolled_out",
        before_rollout_state="unavailable",
        after_rollout_state="healthy",
        timeout_seconds=60,
        elapsed_seconds=4.2,
        approved_by="cli-operator",
        timestamp="2026-08-06T12:00:00+00:00",
        message="restart triggered -- rollout completed within timeout",
    )
    _mock_restart_execute_wiring(
        monkeypatch, plan=plan, execute_side_effect=lambda *a, **k: rolled_out_result
    )

    result = runner.invoke(
        app, ["restart-execute", "checkout-web", "-n", "checkout", "--approve"]
    )

    assert result.exit_code == 0
    assert "rolled_out" in result.stdout


def test_restart_execute_approve_exits_1_when_rollout_times_out(monkeypatch):
    plan = _plannable_restart_plan()
    timed_out_result = RestartResult(
        namespace="checkout",
        name="checkout-worker",
        status="restarted",
        rollout_outcome="timed_out",
        before_rollout_state="unavailable",
        after_rollout_state="unavailable",
        timeout_seconds=60,
        elapsed_seconds=60.0,
        approved_by="cli-operator",
        timestamp="2026-08-06T12:00:00+00:00",
        message="restart triggered -- rollout did NOT complete within timeout "
        "(the restart alone did not fix this; see rollback guidance)",
    )
    _mock_restart_execute_wiring(
        monkeypatch, plan=plan, execute_side_effect=lambda *a, **k: timed_out_result
    )

    result = runner.invoke(
        app, ["restart-execute", "checkout-worker", "-n", "checkout", "--approve"]
    )

    assert result.exit_code == 1
    assert "timed_out" in result.stdout
    assert "rollback guidance" in result.stdout


def test_restart_execute_approve_exits_1_when_cooling_down(monkeypatch):
    plan = _plannable_restart_plan()

    def _raise_cooling_down(*a, **k):
        raise RestartCoolingDownError(plan, 87.3)

    _mock_restart_execute_wiring(
        monkeypatch, plan=plan, execute_side_effect=_raise_cooling_down
    )
    ...
    assert result.exit_code == 1
    assert "restarted less than" in result.output


def test_restart_execute_passes_through_not_allowed_without_requiring_approve(
    monkeypatch,
):
    plan = _not_allowed_restart_plan()
    not_allowed_result = RestartResult(
        namespace="billing",
        name="invoices",
        status="not_allowed",
        reason=plan.reason,
    ...
    _mock_restart_execute_wiring(
        monkeypatch, plan=plan, execute_side_effect=lambda *a, **k: not_allowed_result
    )

    result = runner.invoke(app, ["restart-execute", "invoices", "-n", "billing"])

    assert result.exit_code == 1
    assert "not_allowed" in result.stdout


def test_restart_execute_exits_2_when_kubeconfig_cannot_be_loaded(monkeypatch):
    def _raise_config_error(**kwargs):
        raise ConfigException("Invalid kube-config file. No configuration found.")
...
def _sample_incident_report(*, sources_failed=None):
    ...
    return IncidentContextReport(
        ...
        ownership={
            "status": "OK",
            "detail": "payments [prod/us-east-1] owner=payments-team",
        },
        source_changes={
            "status": "CLEAN",
        ...
        kubernetes={"status": "healthy", "detail": "payments (default): ready=2/2"},
        cloud={"status": "OK", "detail": "0 finding(s) -- 0 active, 0 suppressed"},
        observability={
            "status": "OK",
        ...
        timeline=[
            {"timestamp": "2026-08-01T10:00:00Z", "source": "ci", "summary": "run #1"}
        ],
        sources_ok=[
            "ownership",
            "source_changes",
...
def test_incident_collect_exits_0_on_full_success(monkeypatch):
    monkeypatch.setattr(
        cli_module,
        "collect_incident_context",
        lambda **kwargs: _sample_incident_report(),
    )

    result = runner.invoke(app, ["incident-collect", "payments", "--owner", "example"])

    assert result.exit_code == 0
    assert "[OK] ownership" in result.stdout
    assert "[healthy] kubernetes" in result.stdout
    assert "timeline:" in result.stdout


def test_incident_collect_exits_1_and_lists_unreachable_sources_on_partial_outage(
    monkeypatch,
):
    report = _sample_incident_report(sources_failed=["kubernetes", "cloud"])
    report.kubernetes = {"status": "UNKNOWN", "detail": "kubeconfig not found"}
    report.cloud = {"status": "UNKNOWN", "detail": "connection refused"}
    monkeypatch.setattr(cli_module, "collect_incident_context", lambda **kwargs: report)

    result = runner.invoke(app, ["incident-collect", "payments", "--owner", "example"])

    assert result.exit_code == 1
    assert "unreachable source(s): kubernetes, cloud" in result.stdout


def test_incident_collect_json_output_reports_partial_status(monkeypatch):
    report = _sample_incident_report(sources_failed=["cloud"])
    monkeypatch.setattr(cli_module, "collect_incident_context", lambda **kwargs: report)

    result = runner.invoke(
        app, ["incident-collect", "payments", "--owner", "example", "--json"]
    )
    ...
    assert payload["sources_failed"] == ["cloud"]


def test_incident_collect_requires_owner():
    result = runner.invoke(app, ["incident-collect", "payments"])

    assert result.exit_code != 0


def test_api_serve_is_wired_into_the_cli_help():
...
def test_api_serve_calls_uvicorn_run_with_the_created_app(monkeypatch, tmp_path):
    keys_path = tmp_path / "api-keys.yaml"
    keys_path.write_text("keys:\n  test-key: test-caller\n")
    audit_log = tmp_path / "api-audit.jsonl"
    ...
    assert captured["application"].state.api_keys == {"test-key": "test-caller"}


AI_SERVICE_YAML_TEXT = (
...
def _fake_ai_workload_report(*, verdict="healthy"):
    from platformops.ai_inspect import AIWorkloadReport

    model_status = "PASS" if verdict == "healthy" else "FAIL"
    return AIWorkloadReport(
        ...
        model={
            "status": model_status,
            "detail": "model version is READY (stage=Production)",
        },
        run={"status": "PASS", "detail": "training run FINISHED"},
        endpoint={"status": "PASS", "detail": "endpoint healthy -- 200 in 42.0ms"},
        verdict=verdict,
        sources_ok=["model", "run", "endpoint"],
        sources_failed=[] if verdict == "healthy" else ["model"],
    )


def test_ai_inspect_exits_0_for_a_healthy_workload(tmp_path, monkeypatch):
    service_path = tmp_path / "service-support-assistant.ai.yaml"
    service_path.write_text(AI_SERVICE_YAML_TEXT)
    monkeypatch.setattr(
        cli_module, "inspect_ai_workload", lambda *a, **k: _fake_ai_workload_report()
    )

    result = runner.invoke(app, ["ai-inspect", str(service_path)])

    assert result.exit_code == 0
    assert "HEALTHY" in result.stdout


def test_ai_inspect_exits_1_for_an_unhealthy_workload(tmp_path, monkeypatch):
    service_path = tmp_path / "service-support-assistant.ai.yaml"
    service_path.write_text(AI_SERVICE_YAML_TEXT)
    monkeypatch.setattr(
        cli_module,
        "inspect_ai_workload",
        lambda *a, **k: _fake_ai_workload_report(verdict="unhealthy"),
    )
...
def test_ai_inspect_json_prints_the_full_report(tmp_path, monkeypatch):
    service_path = tmp_path / "service-support-assistant.ai.yaml"
    service_path.write_text(AI_SERVICE_YAML_TEXT)
    monkeypatch.setattr(
        cli_module, "inspect_ai_workload", lambda *a, **k: _fake_ai_workload_report()
    ...
    result = runner.invoke(app, ["ai-inspect", str(service_path), "--json"])
    payload = json.loads(result.stdout)

    assert result.exit_code == 0
    assert payload["verdict"] == "healthy"
    assert payload["registered_model_name"] == "support-assistant-intent-classifier"
    assert payload["model_version"] == "4"


def test_ai_inspect_on_an_invalid_service_definition_exits_2(tmp_path):
    service_path = tmp_path / "service-bad.ai.yaml"
    service_path.write_text("name: support-assistant\n")

    result = runner.invoke(app, ["ai-inspect", str(service_path)])

    assert result.exit_code == 2
    assert "not a valid AI service definition" in result.output


def test_ai_inspect_on_a_missing_file_exits_2():
    result = runner.invoke(app, ["ai-inspect", "no-such-file.yaml"])
...
def _fake_ai_release_readiness_report(*, verdict="ready"):
    from platformops.ai_release_readiness import AIReleaseReadinessReport

    quality_verdict = "ready" if verdict == "ready" else "not_ready"
    return AIReleaseReadinessReport(
        ...
        evaluation={
            ...
            "detail": "accuracy=0.94 meets the required threshold 0.9",
        },
        endpoint={"status": "PASS", "detail": "endpoint healthy -- 200 in 42.0ms"},
        deployment={
            "status": "PASS",
            "detail": "rollout healthy, resource requests set",
        },
        version_match={
            "status": "PASS",
            "detail": "deployment is running the claimed model version (4)",
        },
        model_quality_verdict=quality_verdict,
        infrastructure_verdict="ready",
        verdict=verdict,
        sources_ok=["model", "evaluation", "kubernetes", "endpoint"],
        sources_failed=[],
    )


def test_ai_release_check_exits_0_for_a_ready_release(tmp_path, monkeypatch):
    service_path = tmp_path / "service-support-assistant.ai.yaml"
    service_path.write_text(AI_SERVICE_YAML_TEXT)
    monkeypatch.setattr(
        cli_module,
        "check_ai_release_readiness",
        lambda *a, **k: _fake_ai_release_readiness_report(),
    )

    result = runner.invoke(app, ["ai-release-check", str(service_path)])

    assert result.exit_code == 0
    assert "READY" in result.stdout


def test_ai_release_check_exits_1_for_a_not_ready_release(tmp_path, monkeypatch):
    service_path = tmp_path / "service-support-assistant.ai.yaml"
    service_path.write_text(AI_SERVICE_YAML_TEXT)
    monkeypatch.setattr(
        cli_module,
        "check_ai_release_readiness",
        lambda *a, **k: _fake_ai_release_readiness_report(verdict="not_ready"),
    )
...
def test_ai_release_check_json_prints_both_sub_verdicts(tmp_path, monkeypatch):
    service_path = tmp_path / "service-support-assistant.ai.yaml"
    service_path.write_text(AI_SERVICE_YAML_TEXT)
    monkeypatch.setattr(
        cli_module,
    ...
    result = runner.invoke(app, ["ai-release-check", str(service_path), "--json"])
    payload = json.loads(result.stdout)

    assert result.exit_code == 0
    assert payload["verdict"] == "ready"
    assert payload["model_quality_verdict"] == "ready"
    assert payload["infrastructure_verdict"] == "ready"
    assert payload["deployed_version"] == "4"


def test_ai_release_check_on_an_invalid_service_definition_exits_2(tmp_path):
    service_path = tmp_path / "service-bad.ai.yaml"
    service_path.write_text("name: support-assistant\n")
...
def test_ai_release_check_on_a_missing_file_exits_2():
    result = runner.invoke(app, ["ai-release-check", "no-such-file.yaml"])
