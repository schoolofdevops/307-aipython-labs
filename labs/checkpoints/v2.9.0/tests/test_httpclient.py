...
OBS_FIXTURES = Path(__file__).parent / "fixtures" / "observability"
MLFLOW_FIXTURES = Path(__file__).parent / "fixtures" / "mlflow"


def _obs_fixture(name: str) -> dict:
    return json.loads((OBS_FIXTURES / name).read_text())


def _mlflow_fixture(name: str) -> dict:
    return json.loads((MLFLOW_FIXTURES / name).read_text())
...
def test_list_workflow_runs_follows_link_header_pagination(monkeypatch):
    ...
    assert runs[0]["created_at"] == "2026-08-01T10:00:00Z"
...
# ---------------------------------------------------------------------------
# get_pull_request, list_check_runs, list_artifacts, get_latest_release,
# list_deployments -- Module 26's five new GitHub endpoints. Each follows
# get_repo_info()'s exact respx-mocked pattern; fixture bodies live in
# tests/fixtures/github/ and are shared with test_releasecheck.py.
# ---------------------------------------------------------------------------


@respx.mock
def test_get_pull_request_returns_expected_fields_for_a_merged_pr():
    respx.get("https://api.github.com/repos/acme-platform/payments/pulls/482").mock(
        return_value=httpx.Response(200, json=_fixture("pr_approved.json"))
    )

    pr = get_pull_request("acme-platform", "payments", 482)

    assert pr["number"] == 482
    assert pr["merged"] is True
    assert pr["state"] == "closed"
    assert pr["head"]["ref"] == "retry-budget"


@respx.mock
def test_get_pull_request_raises_endpoint_status_error_on_404(monkeypatch):
    monkeypatch.setattr("platformops.httpclient.time.sleep", lambda _: None)
    respx.get("https://api.github.com/repos/acme-platform/payments/pulls/9999").mock(
        return_value=httpx.Response(404, json={"message": "Not Found"})
    )

    with pytest.raises(EndpointStatusError) as exc_info:
        get_pull_request("acme-platform", "payments", 9999)

    assert exc_info.value.status_code == 404


@respx.mock
def test_list_check_runs_returns_name_status_conclusion():
    respx.get(
        "https://api.github.com/repos/acme-platform/payments/commits/main/check-runs"
    ).mock(return_value=httpx.Response(200, json=_fixture("check_runs_pending.json")))

    runs = list_check_runs("acme-platform", "payments", "main")
...
@respx.mock
def test_list_check_runs_raises_response_format_error_when_key_is_missing():
    respx.get(
        "https://api.github.com/repos/acme-platform/payments/commits/main/check-runs"
    ).mock(return_value=httpx.Response(200, json={"total_count": 0}))

    with pytest.raises(ResponseFormatError):
        list_check_runs("acme-platform", "payments", "main")


@respx.mock
def test_list_artifacts_reports_expired_flag_honestly():
    respx.get(
        "https://api.github.com/repos/acme-platform/payments/actions/artifacts"
    ).mock(return_value=httpx.Response(200, json=_fixture("artifacts_present.json")))

    artifacts = list_artifacts("acme-platform", "payments")

    assert artifacts == [
        {"name": "payments-build", "size_in_bytes": 15423104, "expired": False}
    ]


@respx.mock
def test_list_artifacts_returns_empty_list_when_none_exist():
    respx.get(
        "https://api.github.com/repos/acme-platform/payments/actions/artifacts"
    ).mock(return_value=httpx.Response(200, json=_fixture("artifacts_missing.json")))

    assert list_artifacts("acme-platform", "payments") == []


@respx.mock
def test_get_latest_release_returns_tag_and_published_at():
    respx.get(
        "https://api.github.com/repos/acme-platform/payments/releases/latest"
    ).mock(return_value=httpx.Response(200, json=_fixture("release_latest.json")))

    release = get_latest_release("acme-platform", "payments")

    assert release["tag_name"] == "v1.4.2"
    assert release["published_at"] == "2026-07-30T10:15:00Z"


@respx.mock
def test_get_latest_release_raises_endpoint_status_error_on_404_no_releases(
    monkeypatch,
):
    monkeypatch.setattr("platformops.httpclient.time.sleep", lambda _: None)
    respx.get(
        "https://api.github.com/repos/acme-platform/brand-new-service/releases/latest"
    ).mock(return_value=httpx.Response(404, json={"message": "Not Found"}))

    with pytest.raises(EndpointStatusError) as exc_info:
        get_latest_release("acme-platform", "brand-new-service")

    assert exc_info.value.status_code == 404


@respx.mock
def test_list_deployments_returns_the_payload_image_metadata():
    respx.get("https://api.github.com/repos/acme-platform/payments/deployments").mock(
        return_value=httpx.Response(200, json=_fixture("deployment.json"))
    )

    deployments = list_deployments("acme-platform", "payments")

    assert len(deployments) == 1
    assert deployments[0]["environment"] == "production"
    assert deployments[0]["payload"]["image"] == "ghcr.io/acme-platform/payments:1.4.2"


@respx.mock
def test_list_deployments_raises_response_format_error_when_body_is_not_a_list():
    respx.get("https://api.github.com/repos/acme-platform/payments/deployments").mock(
        return_value=httpx.Response(200, json={"not": "a list"})
    )

    with pytest.raises(ResponseFormatError):
        list_deployments("acme-platform", "payments")


# ---------------------------------------------------------------------------
# query_metrics, search_logs, list_alerts -- Module 30's three new
# observability endpoints.
# ---------------------------------------------------------------------------

OBS_BASE_URL = "http://localhost:9090"
LOGS_BASE_URL = "http://localhost:3100"
ALERTS_BASE_URL = "http://localhost:9093"


@respx.mock
def test_query_metrics_returns_the_matched_samples():
    respx.get(f"{OBS_BASE_URL}/api/v1/query").mock(
        return_value=httpx.Response(
            200, json=_obs_fixture("metrics_query_success.json")
        )
    )

    samples = query_metrics(OBS_BASE_URL, 'http_requests_total{service="payments"}')

    assert len(samples) == 2
    assert samples[0]["metric"]["service"] == "payments"
    assert samples[0]["value"] == (1722900000, "42")


@respx.mock
def test_query_metrics_returns_an_empty_list_when_no_series_match():
    respx.get(f"{OBS_BASE_URL}/api/v1/query").mock(
        return_value=httpx.Response(200, json=_obs_fixture("metrics_query_empty.json"))
    )

    assert query_metrics(OBS_BASE_URL, 'http_requests_total{service="unknown"}') == []


@respx.mock
def test_query_metrics_raises_response_format_error_on_a_bad_status_envelope():
    respx.get(f"{OBS_BASE_URL}/api/v1/query").mock(
        return_value=httpx.Response(200, json={"status": "success"})
    )

    with pytest.raises(ResponseFormatError):
        query_metrics(OBS_BASE_URL, "up")


@respx.mock
def test_search_logs_returns_matching_entries():
    respx.get(f"{LOGS_BASE_URL}/api/v1/logs/search").mock(
        return_value=httpx.Response(200, json=_obs_fixture("logs_search_success.json"))
    )

    logs = search_logs(
        LOGS_BASE_URL, "service:payments AND correlation_id:req-a1b2c3d4e5f6"
    )

    assert len(logs) == 2
    assert logs[1]["level"] == "ERROR"
    assert logs[1]["correlation_id"] == "req-a1b2c3d4e5f6"


@respx.mock
def test_search_logs_returns_an_empty_list_when_nothing_matches():
    respx.get(f"{LOGS_BASE_URL}/api/v1/logs/search").mock(
        return_value=httpx.Response(200, json=_obs_fixture("logs_search_empty.json"))
    )

    assert search_logs(LOGS_BASE_URL, "correlation_id:req-000000000000") == []


@respx.mock
def test_list_alerts_returns_active_alerts():
    respx.get(f"{ALERTS_BASE_URL}/api/v2/alerts").mock(
        return_value=httpx.Response(200, json=_obs_fixture("alerts_active.json"))
    )

    alerts = list_alerts(ALERTS_BASE_URL, service="payments")
...
@respx.mock
def test_list_alerts_returns_an_empty_list_when_nothing_is_firing():
    respx.get(f"{ALERTS_BASE_URL}/api/v2/alerts").mock(
        return_value=httpx.Response(200, json=_obs_fixture("alerts_none.json"))
    )

    assert list_alerts(ALERTS_BASE_URL, service="payments") == []


@respx.mock
def test_list_alerts_raises_response_format_error_when_body_is_not_a_list():
    respx.get(f"{ALERTS_BASE_URL}/api/v2/alerts").mock(
        return_value=httpx.Response(200, json={"not": "a list"})
    )

    with pytest.raises(ResponseFormatError):
        list_alerts(ALERTS_BASE_URL, service="payments")


@respx.mock
def test_query_metrics_is_unreachable_after_retries_are_exhausted(monkeypatch):
    monkeypatch.setattr("platformops.httpclient.time.sleep", lambda _: None)
    respx.get(f"{OBS_BASE_URL}/api/v1/query").mock(
        side_effect=httpx.ConnectError("connection refused")
    )
...
# ---------------------------------------------------------------------------
# get_registered_model / get_model_version / get_run -- MLflow's own REST API
# shape (Model Registry + Tracking). Module 33 talks to these the same way
# query_metrics() talks to Prometheus's shape: real endpoint paths, a real
# response envelope, validated the moment it crosses into this program.
# ---------------------------------------------------------------------------
...
@respx.mock
def test_get_registered_model_returns_the_latest_version_summary():
    respx.get(f"{MLFLOW_BASE_URL}/api/2.0/mlflow/registered-models/get").mock(
        return_value=httpx.Response(
            200, json=_mlflow_fixture("registered_model_found.json")
        )
    )

    model = get_registered_model(MLFLOW_BASE_URL, "support-assistant-intent-classifier")

    assert model["name"] == "support-assistant-intent-classifier"
    assert len(model["latest_versions"]) == 1
    assert model["latest_versions"][0]["version"] == "4"
    assert model["latest_versions"][0]["current_stage"] == "Production"


@respx.mock
def test_get_registered_model_raises_endpoint_status_error_on_404(monkeypatch):
    monkeypatch.setattr("platformops.httpclient.time.sleep", lambda _: None)
    respx.get(f"{MLFLOW_BASE_URL}/api/2.0/mlflow/registered-models/get").mock(
    ...
    with pytest.raises(EndpointStatusError) as excinfo:
        get_registered_model(MLFLOW_BASE_URL, "no-such-model")
    assert excinfo.value.status_code == 404


@respx.mock
def test_get_registered_model_raises_response_format_error_on_malformed_body():
    respx.get(f"{MLFLOW_BASE_URL}/api/2.0/mlflow/registered-models/get").mock(
        return_value=httpx.Response(200, json={"unexpected": "shape"})
    )

    with pytest.raises(ResponseFormatError):
        get_registered_model(MLFLOW_BASE_URL, "support-assistant-intent-classifier")


@respx.mock
def test_get_model_version_returns_status_and_run_id():
    respx.get(f"{MLFLOW_BASE_URL}/api/2.0/mlflow/model-versions/get").mock(
        return_value=httpx.Response(
            200, json=_mlflow_fixture("model_version_ready.json")
        )
    )

    version = get_model_version(
        MLFLOW_BASE_URL, "support-assistant-intent-classifier", "4"
    )

    assert version["status"] == "READY"
    assert version["run_id"] == "a1b2c3d4e5f647a8b9c0d1e2f3a4b5c6"


@respx.mock
def test_get_model_version_reports_a_pending_registration_honestly():
    respx.get(f"{MLFLOW_BASE_URL}/api/2.0/mlflow/model-versions/get").mock(
        return_value=httpx.Response(
            200, json=_mlflow_fixture("model_version_pending.json")
        )
    )

    version = get_model_version(
        MLFLOW_BASE_URL, "support-assistant-intent-classifier", "5"
    )

    assert version["status"] == "PENDING_REGISTRATION"


@respx.mock
def test_get_run_returns_status_and_metrics():
    respx.get(f"{MLFLOW_BASE_URL}/api/2.0/mlflow/runs/get").mock(
        return_value=httpx.Response(200, json=_mlflow_fixture("run_finished.json"))
    )

    run = get_run(MLFLOW_BASE_URL, "a1b2c3d4e5f647a8b9c0d1e2f3a4b5c6")

    assert run["status"] == "FINISHED"
    assert {m["key"]: m["value"] for m in run["metrics"]} == {
        "accuracy": 0.94,
        "f1_score": 0.91,
    }
...
@respx.mock
def test_get_run_reports_a_failed_run_honestly_not_as_an_http_error():
    respx.get(f"{MLFLOW_BASE_URL}/api/2.0/mlflow/runs/get").mock(
        return_value=httpx.Response(200, json=_mlflow_fixture("run_failed.json"))
    )

    run = get_run(MLFLOW_BASE_URL, "b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7")
...
@respx.mock
def test_get_run_retries_a_503_and_succeeds_on_the_next_attempt(monkeypatch):
    monkeypatch.setattr("platformops.httpclient.time.sleep", lambda _: None)
    route = respx.get(f"{MLFLOW_BASE_URL}/api/2.0/mlflow/runs/get")
    route.side_effect = [
        httpx.Response(503),
        httpx.Response(200, json=_mlflow_fixture("run_finished.json")),
    ]
