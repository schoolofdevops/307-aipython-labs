...
## [2.9.0] - AI Release Readiness
### Added
- `src/platformops/kubernetes_inspect.py` -- `DeploymentResourceProfile` and
  `get_deployment_resource_profile()`, a second, deliberately separate read of a Deployment: its
  labels (specifically a `model-version` label) and each container's resource requests
  (cpu/memory/`nvidia.com/gpu`). Kept apart from `DeploymentStatus`/`get_deployment_status()` so a
  caller that only wants rollout health never pays for labels or resource data it did not ask for.
  Read-only -- no new API verb, proven the same way the rest of the module already is.
- `src/platformops/ai_release_readiness.py` -- `platformops.ai_release_readiness`, composing M26's
  fetch/aggregate shape, M33's `ai_inspect.gather_model_evidence()`/`gather_endpoint_evidence()`, and
  M28's `kubernetes_inspect` reads into one release-readiness verdict for an AI-serving workload.
  `gather_evaluation_evidence()` reads a training run's logged metric against a threshold (not just
  whether the run finished); `gather_kubernetes_evidence()` fetches rollout health, labels and
  resource requests, honestly `UNKNOWN` if the cluster is unreachable. `evaluate_ai_release_readiness()`
  is pure -- no network call anywhere in its body -- and keeps two sub-verdicts:
  `model_quality_verdict` (model registry state + evaluation score) and `infrastructure_verdict`
  (deployment rollout, resource requests, deployed-version match, endpoint health), so a low
  evaluation score is never confused with an unhealthy rollout. `deployed_version` -- read from the
  Deployment's own `model-version` label -- is always surfaced, matched or not: the rollback target.
  Makes no write call of any kind.
- `ai-release-check` CLI command -- `platformops ai-release-check service-support-assistant.ai.yaml`
  combines model registry state, evaluation score, Kubernetes deployment health and endpoint health
  into one verdict. Exit 0 for `ready`, 1 for `not_ready`, 2 when the file itself is not a valid AI
  service definition.
- `k8s/support-assistant-namespace.yaml`, `k8s/support-assistant-ready.yaml`,
  `k8s/support-assistant-no-resources.yaml` -- lab scenario manifests for a model-serving Deployment,
  correctly versioned and resourced, and its unresourced contrast case.
- `tests/fixtures/mlflow/run_finished_low_accuracy.json` -- a `FINISHED` run whose logged `accuracy`
  falls below a release threshold, for the model-quality-problem lab scenario.

## [2.8.0] - AI Workload Inspector
### Added
- `src/platformops/aiservice.py` -- `AIServiceDefinition`, a new service-definition model for an
  AI-serving workload: `model_registry`, `registered_model_name`, `model_version` (a plain digit
  string, never `"latest"` or `"vN"`), `serving_runtime`, `endpoint`, `inference_mode` (`"batch"` or
  `"online"`). A new, separate model next to `ServiceDefinition` (M5), not new fields bolted onto
  it. `validate_ai_service()` mirrors `validate_service()`'s never-raises contract exactly.
- `src/platformops/httpclient.py` -- three new read-only functions against MLflow's own documented
  REST API shape: `get_registered_model()`, `get_model_version()`, `get_run()`. Every one of them is
  a single `GET`; the section never issues a `POST`, `PUT`, `DELETE` or `PATCH`. This course does not
  assume a running MLflow tracking server -- tests replay saved example responses instead of a live
  one, the same offline pattern M26 and M30 already use for GitHub and Prometheus/Alertmanager.
- `src/platformops/ai_inspect.py` -- `platformops.ai_inspect`, a fetch/aggregate inspector for an AI
  workload: is the exact model version `READY`, did its training run `FINISH` cleanly, and -- for an
  `online` workload only -- is its serving endpoint healthy. Three `gather_*_evidence()` functions
  fetch; `evaluate_ai_workload()` judges, purely, with no network call anywhere in its body. A batch
  workload's endpoint section reports `NOT_APPLICABLE`, never `FAIL` -- it was never meant to answer
  on a live endpoint. Makes no write call of any kind -- proven by grep.
- `ai-inspect` CLI command -- `platformops ai-inspect service-support-assistant.ai.yaml` reports a
  model version's registry state, its training run's outcome, and its serving endpoint's health.
  Exit 0 for `healthy`, 1 for `unhealthy`, 2 when the file itself is not a valid AI service
  definition.
- `service-support-assistant.ai.yaml` -- example AI service definition.
- `tests/fixtures/mlflow/` -- saved example MLflow Model Registry and Tracking API responses.

## [2.7.0] - PlatformOps Internal API
### Added
- `src/platformops/api.py` -- a FastAPI internal API exposing five of this project's read paths as
  a self-service, stable HTTP contract: `POST /services/validate`, `GET /health`,
  `GET /release-readiness`, `GET /incident-context`, `POST /remediations/plan`. Every route handler
  is a thin wrapper -- it calls the EXACT SAME function the matching CLI command already calls
  (`config.load_yaml_dict()` + `servicedef.validate_service()`, `releasecheck.run_release_check()`,
  `incidentcontext.collect_incident_context()`, `cloudremediate.build_remediation_plan()`) and
  shapes the result into a Pydantic response model. No route re-implements any domain logic.
- `X-API-Key` authentication (`require_api_key()`) -- every route except `GET /health` requires a
  valid key, checked against a `{key: caller_name}` mapping loaded from `api-keys.example.yaml`
  (`load_api_key_config()`).
- A JSON-lines audit log for every request this server receives, successful or not -- method, path,
  caller identity (or `anonymous`), status code, duration -- written through
  `cloudremediate.append_audit_log()`, the same append-only audit-log function M25's remediation and
  M29's governed restarts already use.
- `POST /remediations/plan` is plan-only, by construction: this module never imports
  `cloudremediate.execute_remediation_plan()` or `execute_remediation_batch()`. There is no
  `/remediations/execute` route in this release.
- `api-serve` CLI command -- `platformops api-serve` starts the API server (uvicorn) on
  `127.0.0.1:8080` by default.
- `api-keys.example.yaml` -- example API-key config.

## [2.6.0] - Incident Context Collector
### Added
- `src/platformops/incidentcontext.py` -- a thin orchestrator that composes six already-tested
  adapters into one incident-context report: `local_ops.git_status()`/`git_log_detailed()` (M11),
  `httpclient.list_workflow_runs()` (M9/M26), `kubernetes_inspect.inspect_workload()` (M28),
  `cloudaudit.run_cloud_audit()` (M24), `observability.inspect_observability()` (M30), and
  `config.load_service_yaml()`/`load_yaml_dict()` (M6). Seven `gather_*_evidence()` functions fetch;
  `evaluate_incident_context()` judges, purely, with no I/O. Every section reports a real, known
  status or `UNKNOWN` (could not be reached) -- never a silent gap, never a guess. `build_timeline()`
  merges real timestamps from git commits, CI runs and Kubernetes warning events into one
  time-ordered list. Makes no write call anywhere -- proven by grep.
- `incident-collect` CLI command -- `platformops incident-collect payments --owner ...` gathers
  ownership, source changes, CI state, Kubernetes health, cloud state, observability and a runbook
  reference into one report. Exit 0 when every source answered, exit 1 when at least one could not
  be reached; the report itself is printed either way.
- `local_ops.git_log_detailed()` -- structured `{sha, authored_at, subject}` commit records (ISO 8601
  author dates), additive alongside the existing plain-text `git_log()`.
- `httpclient.list_workflow_runs()` now also returns each run's `created_at` -- additive, existing
  callers unaffected.
- `incident.example.yaml` -- the runbook-URL and documented SLO-target registry `incident-collect`
  reads per service; an SLO target is displayed alongside collected evidence, never calculated from
  live data (no source in this project measures a real request-success ratio yet).

## [2.5.0] - Observable Automation
### Added
- `src/platformops/telemetry.py` -- structured JSON logging (`configure_json_logging()`) and
  OpenTelemetry tracing (`configure_tracing()`) for this project's own automation, tied together by
  `traced_operation()`: one correlation ID, generated by `new_correlation_id()`, tagged onto both a
  span attribute and a `contextvars.ContextVar` that every structured log line inside the block picks
  up automatically. `configure_tracing()` is idempotent within one process -- it checks whether a real
  `TracerProvider` is already installed before calling `trace.set_tracer_provider()`, which OpenTelemetry
  otherwise allows only once per process.
- `src/platformops/httpclient.py` -- three new read-only observability endpoints: `query_metrics()`
  (Prometheus's own `/api/v1/query` response shape), `search_logs()` (a project-defined log-search
  envelope), and `list_alerts()` (Alertmanager's v2 `/api/v2/alerts` bare-array shape). `_request_with_retry()`
  now accepts an optional `params` dict, passed straight through to `httpx` for URL-encoding.
- `src/platformops/observability.py` -- the fetch/aggregate split from `cloudaudit.py` (M24) and
  `releasecheck.py` (M26), applied to metrics/logs/alerts: `gather_metrics_evidence()`,
  `gather_logs_evidence()` and `gather_alerts_evidence()` fetch; `evaluate_observability_snapshot()`
  judges, purely, with no network call. Every section reports one of `OK`, `EMPTY` (fetched, but no
  data -- a real, known answer), `FIRING` (alerts only), or `UNKNOWN` (could not fetch at all) --
  `EMPTY` is never confused with `UNKNOWN`. `inspect_observability()` wraps the whole run in one
  `traced_operation()` span.
- `observability-inspect` CLI command -- fetches metrics, logs and alerts for one service under one
  correlation ID and reports what each source found; exit 0 unless a source was unreachable.
- `opentelemetry-api` and `opentelemetry-sdk` added as runtime dependencies.
