...
from .rules import Server, _servers_in_environment, count_by_environment


def filter_by_environment(inventory: list[Server], env: str) -> list[Server]:
    """Return only the servers whose `env` matches the one given."""
    return _servers_in_environment(inventory, env)


def sort_by_name(inventory: list[Server]) -> list[Server]:
...
def build_summary(inventory: list[Server]) -> dict[str, Any]:
